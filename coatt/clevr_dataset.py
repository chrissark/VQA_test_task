...
class ClevrDataset(Dataset):
    ...
    def __init__(self, image_dir, question_json_file_path,
        ...
        if collate and dataset_type == 'train':
            with open('./data_clevr/train_enc_idx.npy', 'rb') as f:
                self.enc_idx = pickle.load(f)
        elif collate and dataset_type == 'val':
            with open('./data_clevr/val_enc_idx.npy', 'rb') as f:
                self.enc_idx = pickle.load(f)
    ...
    def __getitem__(self, idx):
        ...
        with open(self.qjson, 'rb') as f:
             dataset = json.load(f)
        
        questions = dataset['questions']
        ques_id = self.ques_ids[idx]
        img_id = questions[idx]['image_index']


        qa = questions[idx]['question']
        qqa = questions[idx]['answer']
        img_name = questions[idx]['image_filename']
